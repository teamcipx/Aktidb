...
export default function AddSpecialGmail() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('special_gmail_accounts').insert([finalData]);
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white rounded-xl border border-slate-200 shadow-sm mt-8 mx-auto max-w-sm text-center">
        ...
        <p className="text-slate-500 mt-2 text-sm">The Special Gmail account has been secured.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 flex flex-col h-full font-sans">
      <div className="flex justify-between items-center bg-white border-b border-rose-200 px-4 md:px-8 py-4 -mx-4 md:-mx-8 -mt-4 md:-mt-8 mb-4 shrink-0">
        <div className="flex items-center space-x-2 text-rose-600">
          <ShieldAlert className="w-5 h-5 font-bold" />
          <h1 className="text-xs font-bold uppercase tracking-wider">Add Special Gmail (High Security)</h1>
        </div>
      </div>

      <div className="bg-white border border-rose-200 rounded-xl overflow-hidden shadow-sm flex-1 flex flex-col min-h-[300px]">
        <div className="flex-1 overflow-y-auto scroll-hide p-4 md:p-8">
          <form id="special-gmail-form" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
            <div className="col-span-full text-xs font-bold uppercase tracking-widest text-rose-600 mb-2 border-b border-rose-100 pb-2">Primary Identity</div>
            ...
            <div className="col-span-full mt-4 text-xs font-bold uppercase tracking-widest text-rose-600 mb-2 border-b border-rose-100 pb-2">High Security & Recovery</div>
            ...
            <Field label="First Channel Name" name="first_channel_name" value={formData.first_channel_name} onChange={handleChange} />
            
            <div className="col-span-full mt-4 text-xs font-bold uppercase tracking-widest text-rose-600 mb-2 border-b border-rose-100 pb-2">Device & Location Auth</div>
            ...
            <div className="col-span-full mt-4 text-xs font-bold uppercase tracking-widest text-rose-600 mb-2 border-b border-rose-100 pb-2">Physical Identities</div>
            ...
            <div className="col-span-full space-y-1.5 mt-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-tighter">Highly Sensitive Note</label>
              <textarea
                name="note"
                value={formData.note}
                onChange={handleChange}
                rows={3}
                className="w-full px-3 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-rose-500 outline-none resize-none font-sans bg-rose-50/30"
    ...
    <div className="space-y-1.5 flex flex-col relative w-full">
      ...
      <div className="relative flex items-center w-full">
        <input
          ...
          onChange={onChange}
          required={required}
          className={`w-full px-3 py-2 ${isPassword ? 'pr-9' : ''} border border-slate-200 rounded text-sm focus:ring-1 focus:ring-rose-500 outline-none text-slate-900 bg-white font-sans`}
        />
        {isPassword && (
          <button 
            type="button" 
