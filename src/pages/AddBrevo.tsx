...
import { CheckCircle2, Send, Eye, EyeOff } from 'lucide-react';

export default function AddBrevo() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('brevo_accounts').insert([formData]);
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-slate-900 rounded-xl border border-slate-800 shadow-sm mt-8 mx-auto max-w-sm text-center">
        ...
        <p className="text-slate-400 mt-2 text-sm">The Brevo account has been added.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4 flex flex-col h-full font-sans">
      <div className="flex justify-between items-center bg-slate-900 border-b border-slate-800 px-4 md:px-8 py-4 -mx-4 md:-mx-8 -mt-4 md:-mt-8 mb-4 shrink-0">
        <div className="flex items-center space-x-2 text-indigo-400">
          ...
          <h1 className="text-xs font-bold uppercase tracking-wider">Add Brevo Account</h1>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-sm flex-1 flex flex-col min-h-[300px]">
        <div className="flex-1 overflow-y-auto scroll-hide p-4 md:p-8">
          <form id="brevo-form" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
            ...
            <Field label="Account Name / Label" name="name" value={formData.name} onChange={handleChange} required placeholder="e.g. Brevo Production" />
            ...
            <div className="col-span-full mt-4 text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 border-b border-slate-800 pb-2">API & SMTP Credentials</div>
            ...
            <div className="col-span-full space-y-1.5 mt-2">
              ...
              <textarea
                ...
                placeholder="Any extra details about this Brevo account..."
              />
            </div>
          </form>
        </div>
        ...
        <div className="p-6 border-t border-slate-800 flex items-center justify-end bg-slate-950">
          ...
          >
            {loading ? 'Saving...' : 'Save Brevo Account'}
          </button>
        </div>
      </div>
    </div>
