...
export default function AddContact() {
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white rounded-xl border border-slate-200 shadow-sm mt-8 mx-auto max-w-sm text-center">
        ...
        <p className="text-slate-500 mt-2 text-sm">The contact has been added to your database.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 flex flex-col h-full font-sans">
      <div className="flex justify-between items-center bg-white border-b border-amber-200 px-4 md:px-8 py-4 -mx-4 md:-mx-8 -mt-4 md:-mt-8 mb-4 shrink-0">
      ...
      <div className="bg-white border border-amber-200 rounded-xl overflow-hidden shadow-sm flex-1 flex flex-col min-h-[300px]">
        <div className="flex-1 overflow-y-auto scroll-hide p-4 md:p-8">
          <form id="add-contact-form" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
            <div className="col-span-full text-xs font-bold uppercase tracking-widest text-amber-600 mb-2 border-b border-amber-100 pb-2">Primary Details</div>
            ...
            <div className="col-span-full mt-4 text-xs font-bold uppercase tracking-widest text-amber-600 mb-2 border-b border-amber-100 pb-2">Additional Info</div>
            ...
            <div className="col-span-full space-y-1.5 flex flex-col">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-tighter block">Address</label>
              <input
                type="text"
                name="address"
                value={formData.address}
                onChange={handleChange}
                className="w-full px-3 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-amber-500 outline-none text-slate-900 bg-white font-sans"
              />
            </div>

            <div className="col-span-full text-xs font-bold uppercase tracking-widest text-slate-400 mb-2 mt-4 border-b border-slate-100 pb-2">Status & Metadata</div>
            ...
            <div className="col-span-full space-y-1.5 mt-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-tighter block">Additional Notes</label>
              <textarea
                name="note"
                value={formData.note}
                onChange={handleChange}
                rows={3}
                className="w-full px-3 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-amber-500 outline-none resize-none font-sans bg-amber-50/10"
