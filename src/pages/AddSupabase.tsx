...
export default function AddSupabase() {
  ...
  const [formData, setFormData] = useState({
    ...
    anon_key: '', db_pass: '', purpose: '', creation_date: '', note: ''
  });
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white rounded-xl border border-slate-200 shadow-sm mt-8 mx-auto max-w-sm text-center">
        ...
        <p className="text-slate-500 mt-2 text-sm">The Supabase account has been added.</p>
      </div>
    );
  }
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm flex-1 flex flex-col min-h-[300px]">
        <div className="flex-1 overflow-y-auto scroll-hide p-4 md:p-8">
          <form id="supabase-form" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
            ...
            <div className="col-span-full mt-4 text-xs font-bold uppercase tracking-widest text-slate-400 mb-2 border-b border-slate-100 pb-2">Project Credentials</div>
